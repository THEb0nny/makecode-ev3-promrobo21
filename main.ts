...
const LW_KI_2S = 0; // Интегральный коэффициент регулятора движения по линии двумя датчиками
...
const LW_KI_LS = 1; // Интегральный коэффициент регулятора движения по линии на левом датчике
...
const LW_KI_RS = 1; // Интегральный коэффициент регулятора движения по линии на правом датчике
const LW_KD_RS = 0; // Дифференциальный коэффициент регулятора движения по линии на правом датчике

const KP_SPIN_TURN = 0.25; // Значение Kp для поворота относительно центра
const KD_SPIN_TURN = 2; // Значение Kd для поворота относительно центра

const KP_PIVOT_TURN = 0.4; // Значение Kp для поворота относительно колеса
const KD_PIVOT_TURN = 2; // Значение Kd для поворота относительно колеса

const WHEELS_D = 56; // Диаметер колёс в мм
const WHEELS_W = 135; // Расстояние между центрами колёс в мм

const LW_CONDITION_DETECT_MAX_ERR = 30; // Максимальная ошибка для определения, что робот движется по линии одним датчиком

const ENC_TURN_MAX_ERR_DIFFERENCE = 10; // Пороговое значения ошибки для регулятора умного поворота, что поворот выполнен в мс
const ENC_TURN_MAX_REG_DIFFERENCE = 10; // Пороговое значение регулятора для определения умного поворота в мс
const ENC_TURN_TIME_DEREG = 150; // Время дорегулирования в умном повороте в мс

const ENC_SPIN_TURN_OUT_TIME = 800; // Максимальное время умного поворота относительно центра в мс
const ENC_PIVOT_TURN_OUT_TIME = 1000; // Максимальное время умного поворота относительно колеса в мс

const DIST_AFTER_INTERSECTION = 50; // Дистанция для проезда после опредения перекрёстка для прокатки в мс
const DIST_ROLLING_MOVE_OUT = 20; // Дистанция для прокатки без торможения на перекрёстке в мс
