...
namespace sensors {
     ...
     */
    //% blockId="GetColorSensorRgbHsvl"
    //% block="**color sensor** $sensor RGB&HSVL from RGB"
    //% block.loc.ru="**датчика цвета** $sensor RGB&HSVL с RGB"
    //% inlineInputMode="inline"
    //% sensor.fieldEditor="images"
    //% sensor.fieldOptions.columns="4"
    //% sensor.fieldOptions.width="300"
    //% weight="88" blockGap="8"
    //% group="Color Sensor"
    export function getColorSensorRgbHsvl(sensor: sensors.ColorSensor): number[][] {
    ...
    //% weight="88" blockGap="8"
    //% group="Color Sensor"
    export function getColorSensorHsvl(sensor: sensors.ColorSensor): number[] {
        const normRgb = getNormalizeRgb(sensor);
        const hsvl = rgbToHsvl(normRgb);
        return hsvl;
    }
     ...
     */
    //% blockId="HsvlToColorNumBoundariesColorSensor"
    ...
    //% group="Color Sensor"
    export function hsvlToColorNumBoundariesColorSensor(newColoredBoundary: number, newWhiteBoundary: number, newBlackBoundary: number, newRedBoundary: number, newOrangeBoundary: number, newBrownBoundary: number, newYellowBoundary: number, newGreenBoundary: number, newBlueBoundary: number, newPurpleBoundary: number): ColorBoundaries {
        return {
